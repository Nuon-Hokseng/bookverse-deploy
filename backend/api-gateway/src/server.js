...
try {
    ...
    logger.warn('Rate limiting will not be available');

    // Fallback rate limiter without Redis
    rateLimiter = rateLimit({
        windowMs: 15 * 60 * 1000,
        max: 100,
        standardHeaders: true,
...
routeConfig.forEach(route => {
    logger.info(`Configuring route: ${route.path} -> ${route.service.url} (Auth: ${route.requiresAuth})`);
